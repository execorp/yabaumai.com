...
class Thumbnail {
     ...
     * @return Thumbnail
     */
    function Thumbnail($fileName, $maxWidth = 240, $maxHeight = 320, $noimg = 1) {
        ...
        if($this->error == true) {
            $this->showErrorImage($maxWidth, $maxHeight, $noimg);
            break;
        }
    }
     ...
     *
     */
    function showErrorImage($maxWidth, $maxHeight, $noimg) {
        header('Content-type: image/jpeg');
        $errImg = ImageCreate($maxWidth,$maxHeight);
        $bgColor = imagecolorallocate($errImg,255,160,160);
        ...
        switch($noimg){
        	case 1:imagestring($errImg,12,($maxWidth/2)-34,$maxHeight/2-7,$this->errmsg,$fgColor1);break;
        	case 2:ImageTTFText($errImg, 10, 20, ($maxWidth/2)-32, $maxHeight/2+16, $fgColor1, "./font/GOTHICB.TTF",$this->errmsg);break;
        	case 3:
        		$file = '../img/noimage' . $maxWidth . 'x' . $maxHeight . '.jpg';
        		if(@filesize($file)){
		        	$errImg = @fopen( $file, "r" );
		        	echo @fread( $errImg, filesize($file) );
					fclose( $errImg );
					exit;
        		}else{
        			ImageTTFText($errImg, 10, 20, ($maxWidth/2)-32, $maxHeight/2+16, $fgColor1, "./font/GOTHICB.TTF",$this->errmsg);
        		}
        	break;
        	default:imagestring($errImg,12,($maxWidth/2)-34,$maxHeight/2-7,$this->errmsg,$fgColor1);
        }
        imagejpeg($errImg);
        imagedestroy($errImg);
    }
     ...
     * @param int $maxHeight
     */
    function resize($maxWidth = 240, $maxHeight = 320, $mode = 1, $anime = 1) {
...
/* 透過対応リサイズ処理拡張 */
    if ( ($this->format == GIF || $this->format == PNG) && $mode != 2 && $anime != 2) {
	    ...
	    if ($trnprt_indx >= 0) {
		    ...
		    $color = imagecolorallocatealpha($this->workingImage, 0, 0, 0, 127);
...
/* 透過対応リサイズ処理拡張 */
	 ...
	 * @param int $percent
	 */
	function resizePercent($percent = 0, $mode = 1) {
	 ...
	 * @param string $name
	 */
	function show($quality=100,$name = '', $mode = 1, $galName = '', $shopName = '', $tel = '') {
		if($mode == 3){
			if($name != '') {
			...
			if($name != '') {
		    	...
		    	$galNameColor = imagecolorallocate($this->newImage,255,0,0);
		    	$shopNameColor = imagecolorallocate($this->newImage,0,0,255);
		    	$telColor = imagecolorallocate($this->newImage,0,0,0);
		    	
		    	$margin = $_GET['mh'] - $this->currentDimensions['height'];
		    	
		    	/*
		    	$galName = mb_convert_encoding('かおり', 'UTF-8', 'SJIS');
		    	$shopName = mb_convert_encoding('プライベートヒロイン', 'UTF-8', 'SJIS');
		    	$tel = mb_convert_encoding('TEL:011-280-6160', 'UTF-8', 'SJIS');
		    	*/
		    	
				if($galName)ImageTTFText($this->newImage, 10, 0, 2, 164-$margin, $galNameColor, "./font/meiryob.ttc", mb_convert_encoding(urldecode($galName), 'UTF-8', 'SJIS'));
				if($shopName)ImageTTFText($this->newImage, 8, 0, 2, 176-$margin, $shopNameColor, "./font/meiryob.ttc", mb_convert_encoding(urldecode($shopName), 'UTF-8', 'SJIS'));
				if($tel)ImageTTFText($this->newImage, 8, 0, 2, 186-$margin, $telColor, "./font/meiryob.ttc", 'TEL ' . $tel);
		        header('Content-type: image/jpeg');
		        ImageJpeg($this->newImage,'',$quality);
		    }
		}else{
