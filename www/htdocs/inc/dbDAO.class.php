...
//クラス概要
//DBへのアクセスを一元管理する
//　09/12/11　ver 0.01　pearDB依存で基本動作作成（takeoka）メモリ使用量が多いため改良の余地大いにあり
...
//deleteRecord(基幹テーブルの主キーの値(str))
//動作　関連テーブル全てから指定された主キーのレコードを一度に削除する。
...
//setSearchParallel(検索値(array), AND・OR指定(str)・省略可)
//  同じ項目に複数の条件を設定する。必ずsetSearchValue(項目,setSearchParallel(配列),'○○Array')の形で使う。
/*
用例：名前に太郎か花子が含まれているものを検索する
$search[] = '太郎';
$search[] = '花子';
$exampleDAO->setSearchValue('name', $exampleDAO->setSearchParallel(), 'aboutArray')
...
//データ操作
    ...
    function deleteRecord($id) {
        ...
        if (is_numeric($id) && $id > 0) {
            $this->_reset();
            $this->_setSearchToTmp();
            $this->setSearchValue($this->primaryLabel, $id);
