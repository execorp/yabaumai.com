...
$MyCount = $db->getOne( "SELECT COUNT( `imgId` ) FROM `whatsNew` WHERE `areaId` = ? AND `shopId` = ? ", array(CLIENT_AREA_ID, CLIENT_SHOP_ID) );
$result  = $db->limitQuery( "SELECT `imgId`, `title`, `areaId`, `comment`, `dateTime`, `file` FROM `whatsNew` WHERE `areaId` = ? AND `shopId` = ? ORDER BY `dateTime` DESC ", $ls, $limit, array(CLIENT_AREA_ID, CLIENT_SHOP_ID) );
    $i = 1;
