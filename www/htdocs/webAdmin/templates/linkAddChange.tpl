...
<form{$form.attributes}>
    ...
    <tr><th class="addGal">{$form.genre.label} <span class="cap">※必{literal}須{/literal}</span></th><td>{$form.genre.html}</td></tr>
    <tr><th class="addGal">{$form.userfile.label}</th><td>{$form.userfile.html}&nbsp;<span class="cap">（ *最大 横88×縦31 ）</span>{if $file}<a href="../inc/picR.php?imgId={$imgId}&t={$tableName}" target="_blank"><img src="../inc/picR.php?imgId={$imgId}&t={$tableName}" border="0">{/if}</td></tr>
