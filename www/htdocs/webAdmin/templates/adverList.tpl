...
{foreach from=$prtData.adver item=value name=result}
            <tr>
                <td vAlign="middle" nowrap="nowrap">{$value.regDateTime|date_format:"%Y/%m/%d %H:%M:%S"}</td>
                <td vAlign="middle" nowrap="nowrap">{$value.shopName}</td>
                <td vAlign="middle" width="100%"><a href="mailto:{$value.eMail}">{$value.eMail}</a></td>
                <td nowrap="nowrap"><form action="./adverAddChange.php" method="post"><input type="submit" name="change" value="詳細"><input type="hidden" name="adverId" value="{$value.adverId}"></form></td>
                <td nowrap="nowrap"><form action="./adverList.php" method="post"><input type="checkbox" name="delChk" value="{$value.adverId}"><input type="submit" name="del" value="削除"><input type="hidden" name="adverId" value="{$value.adverId}"></form></td>
