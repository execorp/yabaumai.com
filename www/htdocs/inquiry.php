...
if( GetMobileCareer(0) == "PC" AND $_GET['s'] != '1' ){
    $form->addElement( 'text', 'eMail', 'メールアドレス', array( "istyle" => 1, "size" => 24 ) );
    $form->addElement( 'text', 'name', '店舗名', array( "istyle" => 1, "size" => 24 ) );
    $form->addElement( 'textarea', 'comment', '本文', array( "istyle" => 1, "rows" => 16, "cols" => 55 ) );
}elseif(GetMobileCareer(0) == "smartPhone"){
    $form->addElement( 'text', 'eMail', 'メールアドレス', array( "istyle" => 1, "size" => 24 ) );
    $form->addElement( 'text', 'name', '店舗名', array( "istyle" => 1, "size" => 24 ) );
    $form->addElement( 'textarea', 'comment', '本文', array( "istyle" => 1, "rows" => 16, "cols" => 35 ) );
}
$form->addElement( 'submit', 'submit', '送信', array("class" => "submit"));
$form->addElement( 'reset', 'reset', 'リセット', array("class" => "reset"));

//デフォルト
$form->setDefaults( $iconListDef );
$form->setDefaults(
    array(
        "eMail" => $eMail ,
        "name" => $shopName ,
...
$form->addRule( 'comment', '本文を入力して下さい。', 'required', null, 'client' );
...
//	    $_to      = $shopMail;
	    ...
	    $_from    = $shopMail;
	    $_subject = $siteNamePrint . "ホームページよりお問い合わせがありました。";

	    /* --- 登録完了のメールを送信 --- */
	    $_body  = $siteName . "\n";
	    $_body .= $domain . "\n";
	    $_body .= $siteNamePrint . "ホームページよりお問い合わせがありました。\n\n";
...
$smarty->display( 'inquiry.tpl' );

//if( GetMobileCareer(0) == "PC" AND $_GET['s'] != '1' ){
//    $smarty->display( 'inquiry.tpl' );
//}elseif(GetMobileCareer(0) == "smartPhone"){
//   $smarty->display( 'inquirySP.tpl' );
//}else{
//    $smarty->assign ( "areaIdSelect", $_GET['areaId'] );
//    mb_convert_variables("SJIS-WIN", "UTF-8", $smarty);
//    $smarty->assign ( "emjArray", $emjArray );
//    $template = $smarty->display("inquiryM.tpl");
//}